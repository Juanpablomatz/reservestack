...
export class RosaPage implements AfterViewInit, OnDestroy {
  ...
  async guardarReservaEnServidor(reserva: any, tipoCorreo?: string) {
    try {
      ...
      console.error('❌ Error de conexión a MySQL Rosa Mexicano:', e);
    }
  }
  ...
  desvincularMesa(mesa: any) {
    ...
    if (confirm(`¿Desvincular la Mesa ${mesa.displayId} y restaurar las mesas originales en Rosa Mexicano?`)) {
      const zona = this.zonaActiva;
  ...
  configurarFormularioReserva() {
    ...
    form?.addEventListener('submit', (e) => {
      ...
      setTimeout(() => {
        if (esEdicion) {
          alert('✅ ¡Datos de la reserva actualizados con éxito en Rosa Mexicano!');
        } else {
          alert('✅ ¡Nueva reserva guardada con éxito en Rosa Mexicano!');
        }
      }, 50);
    });
  }
