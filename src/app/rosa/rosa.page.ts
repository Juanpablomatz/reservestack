...
export class RosaPage implements AfterViewInit, OnDestroy {
  ...
  guardarReservasEnCache() {
    if (Array.isArray(this.todasLasReservas)) {
      localStorage.setItem('rosa_reservas_cache', JSON.stringify(this.todasLasReservas));
    }
  }
  ...
  async guardarReservaEnServidor(reserva: any, tipoCorreo?: string) {
    try {
      const payload = { ...reserva, idRestaurante: 2 };
      if (tipoCorreo) {
      ...
      console.error('Error de conexion a MySQL Rosa Mexicano:', e);
    }
  }
  ...
  async inicializarSistema() {
    ...
    if (!this.socket && typeof io !== 'undefined') {
      try {
        ...
        console.warn('Socket error Rosa:', e);
      }
    }
  ...
  async fusionarMesas(mesaA: any, mesaB: any, esPermanente: boolean) {
    ...
    if (esPermanente) {
      try {
        await this.guardarDisenoPermanente();
        alert(`Fusion permanente guardada en el servidor como Mesa ${mesaFusionada.displayId}.`);
      } catch (error) {
        this.disenoMaestro = JSON.parse(JSON.stringify(this.restaurante));
        alert(`Fusion guardada localmente como permanente.`);
      }
    } else {
      this.guardarLayoutFechaActual();
      alert(`Fusion temporal (solo para la fecha ${this.fechaSeleccionada}) aplicada como Mesa ${mesaFusionada.displayId}.`);
    }

    // Salir del modo combinar y refrescar vista sin necesidad de presionar Guardar Distribucion
    this.modoCombinar = false;
    this.mesaACombinar = null;
    this.modoEdicion = false;
    document.getElementById('aviso-combinar')?.classList.add('oculto');
  ...
  dibujarListaDeReservas(reservas: any[]) {
    ...
    if (activas.length === 0 && finalizadas.length === 0) {
      lista.innerHTML = '<p style="color:var(--text-sec); padding:20px; text-align:center;">No hay reservas hoy en Rosa Mexicano.</p>';
      return;
    }
    ...
    if (finalizadas.length > 0) {
       ...
       separador.innerHTML = '<p style="color:var(--text-sec); font-size:10px; text-align:center; margin: 15px 0 5px 0; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 10px; font-weight:bold;">HISTORIAL ROSA MEXICANO</p>';
