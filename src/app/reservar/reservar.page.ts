...
export class ReservarPage implements OnInit {
  ...
  readonly ID_RESTAURANTE = 2; // Rosa Mexicano
  ...
  async cargarDisenoMesas() {
    try {
      const resp = await fetch(`${this.BASE_URL}/api/restaurantes/${this.ID_RESTAURANTE}/diseno`);
      ...
      console.warn('⚠️ Usando distribución de mesas de Rosa Mexicano local de respaldo.');
    }
  }

  // ⏰ VALIDACIÓN INTELIGENTE DE HORARIOS Y HORAS PASADAS (ROSA MEXICANO)
  validarHorarioServicio(fechaStr: string, horaStr: string): { valido: boolean; mensaje: string } {
    ...
    if (diaSemana === 1) { // Lunes
      return { valido: false, mensaje: 'Rosa Mexicano Cocina se encuentra CERRADO los días Lunes.' };
    }

    // 2. Horarios oficiales por día en Rosa Mexicano
    let horaApertura = '09:00';
    let horaCierre = '21:00';

    if (diaSemana >= 2 && diaSemana <= 4) { // Martes a Jueves (9:00 AM - 9:00 PM)
      horaApertura = '09:00'; horaCierre = '21:00';
    } else if (diaSemana === 5 || diaSemana === 6) { // Viernes y Sábado (9:00 AM - 10:00 PM)
      horaApertura = '09:00'; horaCierre = '22:00';
    } else if (diaSemana === 0) { // Domingo (9:00 AM - 6:00 PM)
    ...
    if (horaStr < horaApertura || horaStr > horaCierre) {
      ...
      return { 
        valido: false, 
        mensaje: `Nuestro horario de atención los ${nomDia}s en Rosa Mexicano es de ${horaApertura} a ${horaCierre} hs. Por favor elige una hora dentro del servicio.` 
      };
    }
  ...
  async buscarMesaDisponible(personasRequeridas: number): Promise<number> {
    try {
      const resp = await fetch(`${this.BASE_URL}/api/restaurantes/${this.ID_RESTAURANTE}/reservas`);
      ...
      console.error('Error en algoritmo de asignación de mesa:', error);
    }
  ...
  async confirmarReservacion() {
    ...
    // 2. Validación estricta de Horarios y Horas Pasadas
    ...
    try {
      const response = await fetch(`${this.BASE_URL}/api/restaurantes/${this.ID_RESTAURANTE}/reservas`, {
        method: 'POST',
        headers: {
